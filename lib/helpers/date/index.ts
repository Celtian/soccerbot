...
const corceDateByRegExp = (date: string, regExp: RegExp, months: { [key: string]: number } ): string => {
  ...
  const year = !Number.isNaN(match.groups.year) && Number(match.groups.year) > 1800 && Number(match.groups.year) < 2200 ?
    Number(match.groups.year) : undefined;
  const month = months[match.groups.month];
  const day = !Number.isNaN(match.groups.day) && Number(match.groups.day) > 0 && Number(match.groups.day) <= 31 ?
    Number(match.groups.day) : undefined;
...
export const coerceDate = (date: string, type: SoccerBotProvider): string => {
  ...
  switch (type) {
    case SoccerBotProvider.TRANSFERMARKT:
      regExp = new RegExp('(?<month>(' + Object.keys(MONTH_SHORT).join('|') + '))\\s(?<day>\\d{1,2}),\\s(?<year>\\d{4})');
